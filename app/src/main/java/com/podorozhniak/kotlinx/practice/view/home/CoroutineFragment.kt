...
class CoroutineFragment : BaseFragment<FragmentCoroutinesBinding>() {
    ...
    /*
    * Кожна корутина виконується в якомусь скоупі, який містить контекст. Цей контекст представляється класом CoroutineContext.
    * Цей контекст - це набір параметрів (джоба, диспатчер, ексепшн хендлер).
    ...
    *
    * Structured concurrency — це підхід, при якому всі корутини запускаються в межах певного CoroutineScope.
    * Завдяки цьому вони автоматично скасовуються разом із цим scope, що допомагає уникнути "висячих" корутин і робить код більш передбачуваним та безпечним.
    *
    ...
    */
    ...
    // coroutineScope { } дозволяє всередині suspend-функції або корутини запускати кілька корутин паралельно й гарантує,
    // що вони завершаться (або скасуються разом) перш ніж виконання піде далі.
    ...
    // globalJob (~329 рядок) виступає як парент для двох дочірніх джоб
